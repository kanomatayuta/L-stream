<template>
  <div id="tab-report" ref="tabReport" class="borer_report">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label for="followers_count">{{ $t('campaign_detail.plays_count') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.regeneration_pr"
                        class="report_input"
                        :formatter="value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                        :parser="value => value.replace(/\''\s?|(,*)/g, '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('regeneration_pr')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label>{{ $t('campaign_detail.playback') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.revenue_rate"
                        class="report_input"
                        :formatter="value => `${value}%`"
                        :parser="value => value.replace('%', '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('revenue_rate')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label>{{ $t('campaign.youtube_average_eg') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.er_avg"
                        class="report_input"
                        :formatter="value => `${value}%`"
                        :parser="value => value.replace('%', '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('er_avg')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label>{{ $t('story.click') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.total_click"
                        class="report_input"
                        :formatter="value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                        :parser="value => value.replace(/\''\s?|(,*)/g, '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('total_click')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label>{{ $t('insight.youtube_click') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.click_rate"
                        class="report_input"
                        :formatter="value => `${value}%`"
                        :parser="value => value.replace('%', '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('click_rate')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label>{{ $t('campaign.youtube_total_cv') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.total_conversion"
                        class="report_input"
                        :formatter="value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')"
                        :parser="value => value.replace(/\''\s?|(,*)/g, '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('total_conversion')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
                  ...
                  >
                    ...
                    >
                      <div class="label_wrapper">
                        <label>{{ $t('campaign.youtube_average_cv') }}</label>
                      </div>
                      <a-input-number
                        v-model="model.conversion_avg"
                        class="report_input"
                        :formatter="value => `${value}%`"
                        :parser="value => value.replace('%', '')"
                        :disabled="campaign.campaign_mode === 1 ? disabled = true : disabled = false"
                        @blur="createReport('conversion_avg')"
                        @change.native="changeInputReport()"
                      />
                    </a-form-model-item>
                  </a-col>
    ...
    <hr />
    <div id="tabs" class="button__edit mt-5">
      <a-button
        html-type="submit"
        type="primary"
        class="min-w-100"
    ...
    <div class="kol-table">
      <table>
        <tr class="table__flex">
          ...
          <th class="table__col-width">
            {{ $t('campaign_detail.number_of_registrants') }}
          </th>
          <th class="table__col-width">
            {{ $t('campaign_detail.average_play') }}
          </th>
          <th class="table__col-width">
            {{ $t('campaign_detail.PR_playback') }}
          </th>
          <th class="table__col-width">
            {{ $t('campaign.youtube_good') }}
          </th>
          <th class="table__col-width">
          ...
          <th class="table__col-width">
            {{ $t('campaign.click_number') }}
          </th>
          <th class="table__col-width">
            {{ $t('insight.youtube_click') }}
          </th>
          <th class="table__col-width">
            {{ $t('campaign.youtube_cv') }}
          </th>
          <th class="table__col-width">
            {{ $t('insight.youtube_cv') }}
          </th>
          <th class="table__col-width">
            {{ $t('campaign_detail.revenue') }}
          </th>
          <th class="table__col-width">
        ...
        >
          ...
          <td class="table__col-width">
            {{ item.post_date ? moment(item.post_date).format(FORMAT_DATE_SLASH) : '' }}
          </td>
          <td class="table__col-width">
            {{ item.post_url }}
          </td>
          ...
          <td class="table__col-width">
            {{ commaSeperate(item.kol.Kol_other_info.views_avg) }}
          </td>
          <td class="table__col-width">
            {{ commaSeperate(item.regeneration_pr) }}
          </td>
          <td class="table__col-width">
          ...
          <td class="table__col-width">
            {{ commaSeperate(item.click_count) }}
          </td>
          <td class="table__col-width">
            {{ showDecimal(item.click_rate) }}{{ $t('kol.percentage') }}
          </td>
          <td class="table__col-width">
            {{ commaSeperate(item.conversion_count) }}
          </td>
          <td class="table__col-width">
            {{ showDecimal(item.conversion_rate) }}{{ $t('kol.percentage') }}
          </td>
          <td class="table__col-width">
            {{ showDecimal(item.earning) }}
          </td>
          <td class="table__col-width">
...
import EditTableYoutubeModal from '~/components/templates/campaign/TabReport/EditTableYoutubeModal'
...
.kol-table{
  /deep/ {
    ...
    td.table__col-width:nth-child(18) {
      width: 5%;
    }
  }
}
...
td.table__col-width.product_name {
  font-family: 'BIZ UDGothic';
}
