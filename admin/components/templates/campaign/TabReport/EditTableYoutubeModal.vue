        ...
        <div class="table-edit">
          <table>
            <tr>
              ...
              <th>{{ $t('campaign.click_number') }}</th>
              <th>{{ $t('insight.youtube_click') }}</th>
              <th>{{ $t('campaign.youtube_cv') }}</th>
              <th>{{ $t('insight.youtube_cv') }}</th>
              <th>{{ $t('campaign_detail.revenue') }}</th>
            ...
            >
              ...
              <td>{{ commaSeperate(item.kol.Kol_other_info.views_avg) }}</td>
              <td>
                <a-input-number
                ...
                >
                  ...
                  <a-menu slot="overlay">
                    ...
                    <a-menu-item>
                      <div
                        class="table-button-child"
                        @click="deleteYoutubeRecord(item, index)"
                      >
                        {{ $t('common.delete') }}
                      </div>
                    </a-menu-item>
...
export default {
  ...
  methods: {
    /**
     * On change data Youtube
     *
     * @param item
     */
    dataYoutubeChange(item) {
      ...
      const clickCount = parseFloat(item.click_count)
      const regenerationPr = parseFloat(item.regeneration_pr)
      const totalCV = parseFloat(item.conversion_count)
      if (isNaN(totalCV) || isNaN(regenerationPr) || regenerationPr === 0) {
        item.conversion_rate = 0
      } else {
        item.conversion_rate = totalCV / regenerationPr * 100
      }
      if (isNaN(like) || isNaN(comment)) {
        item.preg_rate = 0
      } else {
        item.preg_rate = (like + comment) / follower * 100
      }
      if (isNaN(regenerationPr) || isNaN(clickCount) || regenerationPr === 0) {
        item.click_rate = 0
      } else {
        item.click_rate = clickCount / regenerationPr * 100
      }
     ...
     */
    deleteYoutubeRecord(record, index) {
      let count = 0
      this.data.map(item => {
