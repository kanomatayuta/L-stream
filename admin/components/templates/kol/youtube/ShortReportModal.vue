        ...
        <a-card>
          ...
          <div class="poster-box">
            <div class="header-issue">
              <span class="video-title mt-4">{{ $t('kol.video') }}</span>
            </div>
            <Video v-if="media" :medias="postersSortByTime" />
          </div>
          <div class="follower-analysis">
            <img src="/images/UsersBorder.png" />
            <span class="follower-analysis-title">
              {{ $t('kol.follower_analysis_youtube') }}
            </span>
            <a-tooltip placement="top">
              <template slot="title">
                <span>{{ $t('kol.follower_analysis_youtube_issue') }}</span>
              </template>
              <img
                src="/images/Info.png"
                alt="Info"
                      ...
                      <a-row class="title">
                        <a-col class="title-left" :span="12">
                          <div
                            ...
                            :key="index1"
                          >
                            <span v-if="(item.id === country.title.toUpperCase())">{{ item.value }}</span>
                          </div>
                        </a-col>
          ...
          <div class="chart-column box-content">
            <div class="header-issue">
              ...
              <a-tooltip placement="top">
                <template slot="title">
                  <span>{{ $t('kol.age_gender_chart_issue') }}</span>
                </template>
                <img
                  src="/images/Info.png"
                  alt="Info"
          ...
          <div class="poster-box mt-4">
            ...
            <Video v-if="media" :medias="media.posters" />
          </div>
        </a-card>
      </div>
    </a-skeleton>
    <div id="FullReportModal">
      <full-report-modal ref="FullReportModal" :user="user" :metrics="metrics" :features="features" :media="media" />
    </div>
  </a-drawer>
</template>
<script>
...
import FullReportModal from '~/components/templates/kol/youtube/FullReportModal.vue'
import Video from '~/components/templates/listReport/youtube/Video'
...
export default {
  ...
  computed: {
    ...
    audienceGeo() {
      if (this.features && this.features.audience_geo && this.features.audience_geo.data) {
        return this.features.audience_geo.data.slice(0, 3)
      }
      return null
    },
    audienceLanguages() {
    ...
    postersSortByTime() {
      if (this.media && this.media.posters_sort_by_time) {
        return this.media.posters_sort_by_time.slice(0, 2)
      }
      return null
    }
  },
  methods: {
    ...
    async getReport(channel) {
      ...
      try {
        ...
        this.user = data.report.basic
        this.features = data.report.features
        this.metrics = data.report.metrics
